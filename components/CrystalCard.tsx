...
export const CrystalCard: React.FC<CrystalCardProps> = ({ data, isTransitioning, onNavigate, onJump }) => {
  ...
  // 3. 每次打开选择器时，初始化为当前卡片的日期
  useEffect(() => {
    if (pickerOpen) {
      const monthIdx = new Date(Date.parse(data.month + " 1, 2026")).getMonth();
      setSelectedMonth(isNaN(monthIdx) ? 0 : monthIdx);
      setSelectedDay(parseInt(data.date) || 1);
    }
  }, [pickerOpen, data]);

  // Logic Correction: Ensure selectedDay is valid when switching months
  useEffect(() => {
  ...
  // Dynamic Days Array based on selected month
  ...
  const getDateStyles = (mood: Mood) => {
    switch (mood) {
      case 'CRISIS': // 熔岩红 (Lava Red)
        return "bg-gradient-to-b from-white via-red-500 to-red-900 drop-shadow-[0_0_25px_rgba(220,38,38,0.6)]";
      case 'FUTURE': // 切伦科夫蓝 (Cherenkov Blue)
        return "bg-gradient-to-b from-white via-cyan-400 to-blue-600 drop-shadow-[0_0_25px_rgba(8,145,178,0.6)]";
      case 'GLORY': // 荣耀金 (Default)
      default:
        return "bg-gradient-to-b from-white via-white to-white drop-shadow-[0_0_40px_rgba(240,185,11,0.55)]";
    }
  };

  // 根据 isTransitioning 状态，决定是否添加 'animating' 类名
  // UPDATED: Use calc for responsive width with margins, ensuring it hits max-w limit on desktop without shrinking to content
  const containerClass = `crystal-card relative flex flex-col overflow-hidden transition-all duration-500 w-[calc(100%-2rem)] max-w-[380px] ${isTransitioning ? 'animating' : ''}`;
  ...
  return (
    <div 
      ...
      style={{
        // Removed hardcoded width for responsiveness
        height: '680px',
        borderRadius: '32px',
    ...
    >
      {/* 边缘导航热区 */}
      <div 
        className="absolute top-0 bottom-0 left-0 w-[80px] z-40 cursor-pointer group"
        onClick={() => onNavigate(-1)}
      >
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-r from-[rgba(240,185,11,0.15)] to-transparent pointer-events-none" />
      </div>
      <div 
        className="absolute top-0 bottom-0 right-0 w-[80px] z-40 cursor-pointer group"
        onClick={() => onNavigate(1)}
      >
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-l from-[rgba(240,185,11,0.15)] to-transparent pointer-events-none" />
      </div>
      ...
      {/* 增强：加入 Parallax 反光位移 calc(var(--mouse-x) * -20px) */}
      <div 
      ...
      {/* 头部信息 */}
      <div className={`header-bar px-[35px] pt-[35px] flex justify-between items-center z-10 transition-opacity duration-300 ${pickerOpen ? 'opacity-30' : 'opacity-100'}`}>
        <svg 
          className="w-[32px] h-[32px] drop-shadow-[0_0_10px_rgba(240,185,11,0.5)]" 
          viewBox="0 0 32 32" 
        ...
        <div className="font-song text-[16px] text-white font-bold tracking-[2px] shadow-white/20 drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]">
          二零二六 · 知识年鉴
        </div>
      </div>
      ...
      <div className="content-wrapper z-20 h-full flex flex-col">
        ...
        <div className="visual-area relative flex-grow flex flex-col justify-center pl-[35px]">
          ...
          <div className={`date-group relative z-[2] transition-all duration-300 ${pickerOpen ? 'opacity-0 translate-x-4 blur-sm' : 'opacity-100'}`}>
            ...
            <div 
              className={`date-huge font-ui text-[110px] font-extrabold text-transparent bg-clip-text leading-none tracking-[-5px] -ml-[5px] cursor-pointer transition-all duration-500 hover:scale-105 hover:translate-x-2 ${getDateStyles(data.mood)}`}
        ...
        {/* 文本内容区域 */}
        <div 
          className={`content-pane px-[35px] py-[35px] z-[5] flex flex-col transition-all duration-300 ${pickerOpen ? 'opacity-0 translate-y-4 blur-md' : 'opacity-100'}`}
          style={{ 
        ...
        >
          ...
          <div>
            ...
            <h1 className="title-cn font-hei text-[28px] font-bold text-white m-0 mb-[12px] leading-[1.3] drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
              {data.title}
            </h1>
          ...
          <div className="mt-auto">
            ...
            <div className="quote-area text-right">
              <p className="font-song font-bold italic text-[14px] text-white/70 leading-relaxed tracking-wide inline-block drop-shadow-md">
                “{data.quote}”
              </p>
            </div>
          </div>
      ...
      >
         <div className="holo-roller-stage flex gap-[30px] relative perspective-[1000px] h-[240px] w-full justify-center mask-gradient">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[60px] border-t border-b border-binance-yellow/50 bg-binance-yellow/5 shadow-[0_0_30px_rgba(240,185,11,0.1)] pointer-events-none z-0"></div>
            <div className="holo-col w-[80px] h-full overflow-y-auto overflow-x-hidden snap-y snap-mandatory text-center py-[90px] z-10 no-scrollbar">
               {months.map((m, idx) => (
                 <div key={m} className={`holo-item h-[50px] leading-[50px] font-code text-[18px] snap-center cursor-pointer transition-all duration-300 ${selectedMonth === idx ? 'text-binance-yellow text-[24px] font-bold scale-125 shadow-glow' : 'text-white/20 hover:text-white/60'}`} onClick={() => setSelectedMonth(idx)}>{m}</div>
               ))}
            </div>
            <div className="holo-col w-[80px] h-full overflow-y-auto overflow-x-hidden snap-y snap-mandatory text-center py-[90px] z-10 no-scrollbar">
               {days.map((d) => (
                 <div key={d} className={`holo-item h-[50px] leading-[50px] font-code text-[18px] snap-center cursor-pointer transition-all duration-300 ${selectedDay === d ? 'text-binance-yellow text-[24px] font-bold scale-125 shadow-glow' : 'text-white/20 hover:text-white/60'}`} onClick={() => setSelectedDay(d)}>{String(d).padStart(2, '0')}</div>
               ))}
            </div>
         </div>
